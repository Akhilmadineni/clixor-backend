...
checkable_index_count="$(docker exec "${container_name}" sh -ec \
  'PGPASSWORD="$POSTGRES_PASSWORD" exec psql --username restore_admin --dbname restore_drill --no-psqlrc --tuples-only --no-align --set ON_ERROR_STOP=1 --command "SELECT count(*) FROM pg_index i JOIN pg_class idx ON idx.oid = i.indexrelid JOIN pg_class tbl ON tbl.oid = i.indrelid JOIN pg_namespace n ON n.oid = tbl.relnamespace JOIN pg_am am ON am.oid = idx.relam WHERE n.nspname !~ \$\$^pg_\$\$ AND n.nspname <> \$\$information_schema\$\$ AND tbl.relkind IN (\$\$r\$\$, \$\$m\$\$) AND am.amname = \$\$btree\$\$ AND i.indisvalid;"')"
